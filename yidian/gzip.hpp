...
#ifndef MOMARSAPP_GZIP_HPP
#define MOMARSAPP_GZIP_HPP
...
public:
    ...
    size_t compress(InputType &output,
                  const char *data,
                  std::size_t size,
                  uint8_t **_p_output) const {
...
//        deflate_s.next_in = reinterpret_cast<z_const Bytef * > (data);
        ...
        if (*_p_output == nullptr) {
            *_p_output = (uint8_t *) malloc(output.size());
            memcpy(*_p_output, output.c_str(), output.size());
        }
...
inline size_t compress(const char *data,
                            std::size_t size,
                            uint8_t **_p_output,
                            int level = Z_DEFAULT_COMPRESSION) {
    Compressor comp(level);
    std::string output;
    auto out_size = comp.compress(output, data, size, _p_output);
    return out_size;
}
...
public:
    Decompressor(
            std::size_t max_bytes = 1000000000) // by default refuse operation if compressed data is > 1GB
            : max_(max_bytes) {
    }

    template<typename OutputType>
    size_t decompress(OutputType &output,
                    const char *data,
                    std::size_t size,
                    uint8_t **_p_output) const {
        z_stream inflate_s;
...
inline size_t decompress(const char *data, std::size_t size, uint8_t **_p_output) {
    Decompressor decomp;
    std::string output;
    auto out_size = decomp.decompress(output, data, size, _p_output);
    return out_size;
}
